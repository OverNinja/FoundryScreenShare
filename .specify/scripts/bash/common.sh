...
# Check if python3 is available and actually executable/working
has_python3() {
...
get_invoke_separator() {
    ...
    if [[ -f "$integration_json" ]]; then
        ...
        if [[ "$parsed_with_jq" -eq 0 ]] && has_python3; then
...
resolve_template() {
    ...
    if [ -d "$presets_dir" ]; then
        ...
        if [ -f "$registry_file" ] && has_python3; then
...
" 2>/dev/null); then
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        ...
                        if [ -f "$manifest" ] && has_python3; then
